...
if [ "$MODE" = "--pre-deploy" ]; then
  ...
  # Basic JSON syntax (OpenCode — now generated by KCL, still template-type for dotter)
  if [ -f "$REPO_ROOT/out/opencode.json" ]; then
    _SKIP "source JSON validation (template file; will validate after deploy)"
  fi
...
" 2>/dev/null || rc=$?
  ...
  if [ -f "$DEPLOYED/opencode/opencode.json" ]; then
    validate_jsonc_schema "$DEPLOYED/opencode/opencode.json" || FAILED=1
  fi
