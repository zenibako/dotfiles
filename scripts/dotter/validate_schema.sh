...
if [ "$MODE" = "--pre-deploy" ]; then
  ...
  # rendered. Substitute every {{...}} with a bare literal (valid as a value,
  # inside a string, or as a key) before parsing to check structural validity.
  if [ -f "$REPO_ROOT/out/claude_desktop_config.json" ] && has_python3; then
    if _python3 -c "import json,re; s=open('$REPO_ROOT/out/claude_desktop_config.json').read(); json.loads(re.sub(r'\{\{[^}]*\}\}','true',s))" >/dev/null 2>&1; then
