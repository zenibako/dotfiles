...
4. **reviewer** — **Always run reviewer after any local-dev change — this step is never optional, even for tiny or "obvious" changes.** Pass it the commit SHA, branch, or PR ref that represents the work; with no arguments reviewer will diff the current working copy.
